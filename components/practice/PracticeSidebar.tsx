...
export function PracticeSidebar() {
  ...
  return (
    <>
      {/* Mobile: Hamburger button - top right to avoid covering step numbers */}
      <button
        onClick={() => setIsExpanded(true)}
        className="fixed right-4 top-4 z-50 inline-flex h-10 w-10 items-center justify-center rounded-lg border border-zinc-700 bg-zinc-900 text-zinc-300 transition hover:border-zinc-600 hover:bg-zinc-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 lg:hidden"
      ...
      {/* Sidebar - collapsed on desktop, slide-in on mobile */}
      <aside
        className={`fixed left-0 top-0 z-50 h-full border-r border-zinc-800 bg-zinc-950 transition-all duration-300 ${
          isExpanded
            ? "w-64"
            : "w-16 max-lg:w-0 max-lg:-translate-x-full"
        }`}
          ...
          <div className="flex h-16 items-center justify-between border-b border-zinc-800 px-4">
            {isExpanded ? (
              <>
                ...
                <button
                  onClick={() => setIsExpanded(false)}
                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-zinc-400 transition hover:bg-zinc-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 lg:hidden"
