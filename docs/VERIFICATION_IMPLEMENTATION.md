...
1. **Load suggested endpoints from reference JSON**
   ...
   const suggestedEndpoints = reference.apiEndpoints.filter(
     (endpoint) =>
       endpoint.required || !endpoint.requiresFeature || selectedFeatures[endpoint.requiresFeature]
   );
   ```
...
2. **Pass filtered list to Palette:**
   ...
   const paletteItems = COMPONENT_LIBRARY.filter((c) => allowedComponents.includes(c.kind));
   ```
