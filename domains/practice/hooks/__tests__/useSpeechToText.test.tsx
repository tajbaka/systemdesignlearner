...
describe("useSpeechToText", () => {
  ...
  it("returns empty interimText for whisper provider (batch mode only)", async () => {
    const { result } = renderHook(() =>
      useSpeechToText({
        provider: "whisper",
      })
  ...
  it("cancel stops recording without transcription", async () => {
    const onFinal = vi.fn();
    const { result } = renderHook(() =>
      useSpeechToText({
        onFinal,
  ...
  it("auto-detects whisper provider when Web Speech is not available", () => {
    // Ensure Web Speech is not available
    delete (window as unknown as { SpeechRecognition?: unknown }).SpeechRecognition;
    delete (window as unknown as { webkitSpeechRecognition?: unknown }).webkitSpeechRecognition;

    const { result } = renderHook(() => useSpeechToText());

    // Should default to whisper without errors
    expect(result.current.isRecording).toBe(false);
