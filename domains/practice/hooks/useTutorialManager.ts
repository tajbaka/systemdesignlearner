...
import type { PlacedNode, Edge, PracticeDesignState, Requirements } from "@/domains/practice/types";
...
export function getTutorialSteps(requirements: Requirements): TutorialStep[] {
    ...
    },
    {
      ...
      check: ({ nodes }) =>
        nodes.some((node) => {
          ...
          return match && match[1] === "Cache (Redis)";
        }),
    },
    {
      id: "wire-cache",
      ...
      check: ({ nodes, edges }) => {
        ...
        const cache = nodes.find((node) => {
          const match = node.id.match(/^node-([^-]+)-/);
          return match && match[1] === "Cache (Redis)";
        });
        const db = nodes.find((node) => {
          const match = node.id.match(/^node-([^-]+)-/);
          return match && match[1] === "DB (Postgres)";
        });
  ...
  if (requirements.functional["rate-limiting"]) {
    steps.push({
      ...
      auto: true,
      check: ({ nodes, edges }) => {
        const api = nodes.find((node) => {
          const match = node.id.match(/^node-([^-]+)-/);
          return match && match[1] === "API Gateway";
        });
        const service = nodes.find((node) => {
          const match = node.id.match(/^node-([^-]+)-/);
          return match && match[1] === "Service";
        });
        const limiter = nodes.find((node) => {
          const match = node.id.match(/^node-([^-]+)-/);
          return match && match[1] === "Rate Limiter";
        });
