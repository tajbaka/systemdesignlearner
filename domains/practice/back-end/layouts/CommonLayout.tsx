...
import type { ProblemConfig, Hint, DesignSolution } from "../types";
import type { StepHandlers, AllStepActions } from "../types";
import type { StepArticle } from "../components/StepTooltip";
...
function getArticlesForStep(config: ProblemConfig, stepType: string | null): StepArticle[] {
  if (!stepType || stepType === STEPS.INTRO || stepType === STEPS.SCORE) return [];
...
export function CommonLayout({
  ...
  const stepArticles = getArticlesForStep(config, stepType);
      ...
      <TooltipProvider>
        <div className="flex h-full w-full flex-1 flex-col overflow-hidden lg:pl-16">
          ...
          <div className="bg-zinc-950/90 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/80 mx-auto w-full flex-1 flex flex-col min-h-0">
            <div className="flex-shrink-0 md:px-[20%]">
              {pageTitle && (
          ...
          <footer className="bg-black border-t border-zinc-800 flex-shrink-0">
            <div className="mx-auto w-full max-w-5xl px-4 md:max-w-none md:px-[20%]">
              <PracticeFooter
                leftButton={{
                  show: showBack,
                  onClick: handleBack,
