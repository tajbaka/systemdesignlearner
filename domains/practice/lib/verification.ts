import type { Requirements, ApiEndpoint, ApiEndpointConfig } from "@/domains/practice/types";
import type { ProblemScoringConfig } from "@/domains/practice/types";
...
const PROMPT_BUILDERS = {
  functional: (input: FunctionalVerificationInput, context: VerificationContext) => {
    ...
    const allRequirements = scoringConfig.steps.functional.requirements;
    const required = allRequirements.filter((r) => r.required === true);
    const optional = allRequirements
...
Return ONLY valid JSON in this format:
{
  ...
  nonFunctional: (input: NonFunctionalVerificationInput, context: VerificationContext) => {
    const { scenarioTitle, scoringConfig } = context;
    const allRequirements = scoringConfig.steps.nonFunctional.requirements || [];
    const required = allRequirements.filter((r) => r.required === true);
    const optional = allRequirements.filter((r) => r.required === false);
...
Return ONLY valid JSON:
{
  ...
  api: (input: ApiVerificationInput, context: VerificationContext) => {
    ...
    const requiredEndpoints: ApiEndpointConfig[] = [];
    const optionalEndpoints: ApiEndpointConfig[] = [];

    for (const req of api.requirements || []) {
      if (req.endpoint) {
        // Add the requirement id as the endpoint id for compatibility
        const endpoint = { ...req.endpoint, id: req.id };
        if (req.required) {
          requiredEndpoints.push(endpoint);
        } else {
          optionalEndpoints.push(endpoint);
        }
      }
    }

    // Filter optional endpoints based on selected features
    const filteredOptional = optionalEndpoints.filter((e: { requiredBy?: string[] }) => {
    ...
    optionalEndpoints.push(...filteredOptional);

    return `You are verifying API design for a ${scenarioTitle} system.

**Required Endpoints:**
${requiredEndpoints.map((e) => `- ${e.method} ${e.correctPath || "endpoint"}: ${e.purpose}`).join("\n")}

**Optional Endpoints (based on selected features):**
${optionalEndpoints.length > 0 ? optionalEndpoints.map((e) => `- ${e.method} ${e.correctPath || "endpoint"}: ${e.purpose}`).join("\n") : "None"}
