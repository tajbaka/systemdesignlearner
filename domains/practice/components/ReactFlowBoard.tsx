...
import { PlacedNode, Edge, SystemDesignNode, SystemDesignEdge, ComponentKind } from "./types";
import {
...
function ReactFlowBoardInner({
    ...
    setRfNodes((currentNodes) => {
      ...
      const newRfNodes = nodes.map((node) => {
        ...
        if (existingNode) {
          ...
          // Infer kind from node ID (format: "node-{kind}-{id}")
          const match = node.id.match(/^node-([^-]+)-/);
          const kind = (match ? match[1] : "Service") as ComponentKind;
          const rfNode = placedNodeToReactFlowNode(node, kind);
          return {
            ...rfNode,
            data: {
              ...rfNode.data,
  ...
  const handleConnect = React.useCallback(
    (connection: Connection) => {
      ...
      // Also swap handles to match the swapped nodes
      // If we swapped nodes, we need to swap the handles too
      const needsSwap = dragStartNode.current && dragStartNode.current !== connection.source;
      const actualSourceHandle = needsSwap
        ? connection.targetHandle || undefined
        : dragStartHandle.current || connection.sourceHandle || undefined;
      const actualTargetHandle = needsSwap
        ? connection.sourceHandle || undefined
        : connection.targetHandle || undefined;

      const newEdge: SystemDesignEdge = {
        id: `edge-${actualSource}-${actualTarget}-${Date.now()}`,
        source: actualSource,
        target: actualTarget,
