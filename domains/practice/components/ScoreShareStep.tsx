...
import { encodeDesign as encodeShare } from "@/domains/practice/lib/shareLink";
import { usePracticeSession } from "./PracticeSessionProvider";
import { track } from "@/lib/analytics";
import { logger } from "@/lib/logger";
import type { CumulativeScore, FeedbackResult } from "@/domains/practice/types";
import {
  getGradeDescription,
  getGradeColor,
...
const getKindFromNodeId = (id: string): string => {
...
const buildSharePayload = (state: ReturnType<typeof usePracticeSession>["state"]) => ({
  ...
  nodes: state.design.nodes.map((node) => ({
    id: node.id,
    kind: getKindFromNodeId(node.id),
    x: node.x,
    y: node.y,
  })),
...
export function ScoreShareStep() {
  ...
  const getStepResult = useCallback(
    (
      ...
      if (key !== "design") {
        ...
        if (iterative) {
          return iterativeToFeedbackResult(iterative);
        }
      }
  ...
  const cumulativeScore: CumulativeScore | null = useMemo(() => {
    // Check if we have results for required steps (either from scores or iterative feedback)
    const functionalResult = getStepResult("functional", 20);
    const nonFunctionalResult = getStepResult("nonFunctional", 20);
    const apiResult = getStepResult("api", 25);
    const designResult = getStepResult("design", 35);

    // Check if required steps have been completed
    // A step is considered complete if:
    // 1. It has a result in scores, OR
    // 2. It has a cachedResult in iterativeFeedback (which means it was evaluated)
    const hasFunctional =
      scores?.functional !== undefined || state.iterativeFeedback?.functional?.cachedResult != null;
    const hasNonFunctional =
      scores?.nonFunctional !== undefined ||
      state.iterativeFeedback?.nonFunctional?.cachedResult != null;
    const hasApi = scores?.api !== undefined || state.iterativeFeedback?.api?.cachedResult != null;

    // All three required steps must have results
    if (!hasFunctional || !hasNonFunctional || !hasApi) {
      return null;
    }

    return calculateCumulativeScore(functionalResult, nonFunctionalResult, apiResult, designResult);
  }, [scores, state.iterativeFeedback, getStepResult]);
